function [ t1,y1,phiMax,phiratio,phiCrit] = solveODE_buzzbutton( t,Rr,Rhw,l,I,Rfw,w,aR,k,inputForceList,gamma,x1,v1 )
%Initial conditions
initial1 = [x1, v1];
...
%t1 and y1 are the result of solving the differential dx


function dx = paperfugeodeSpring(t,y)  %y is phi and phi dot but where does it come from?
%paperfugeodeSpring
%       uses current position in the paperfuge period to calculate
%       all three torques (input torque, drag torque, spring
%       torque) and update angular position based on ODE
    ...
    dragTorque = -sign(y(2))*dragWheel*(y(2)^2);
    

    %use springTs to calculate the string torque
    if S==1
        twistTorque = -sign(y(1))*springTs(y(1));
    else
        twistTorque = 0;
    end
    ...
    dx = [y(2);                             % Angular velocity
      (1/I)* (inputTorque + dragTorque + twistTorque);    % Angular acceleration
     ]; 
end 
end